function spikeshape(desired_channels_neural, num_stim, numch, y_lim,w_pre,w_post,channel_neural_data,spike_lcs_ss,sample_rate, ave, fecha, file, name_stim, profundidad)
 %Devuelve spike shapes de los canales seleccionados en diferentes
 %subplots, hace una figura por estimulo.
 %Version 06/08/2020
 %Matlab 2017a

 stims=unique(num_stim);
 
for k=1:length(stims) %para cada tipo de estimulo
        for ch=1:numch %y para cada canal
 for m=1:length(spike_lcs_ss{1,stims(k)}) %tomo cada instancia de spike
     hold on
     spikeshapes(:,m)=channel_neural_data(spike_lcs_ss{1,k}(m)-w_pre*sample_rate : spike_lcs_ss{1,k}(m)+w_post*sample_rate,ch); %#ok<AGROW> %y tomo la ventana que yo le setee
 end
...
for ch=1:numch %para cada canal
    ylim(y_lim);
    for m=1:length(spike_lcs_ss{stims(k)}) %ploteo cada spike apilandolos en un plot por canal 
...
%Tabla de datos
 estimulo=name_stim(num_stim==stims(k)); %nombre del estimulo
...
colnames={'Ave', 'Fecha', 'Protocolo', 'Estimulo','Profundidad', 'Spikes'};
valuetable={ave, fecha, file, estimulo, profundidad,numspikes_stim(k)};       
